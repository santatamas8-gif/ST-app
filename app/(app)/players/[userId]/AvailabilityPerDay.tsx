...
export function AvailabilityPerDay({
  ...
  return (
    <div className="overflow-visible space-y-2">
      {dates.map((date) => (
        <div key={date} className="flex min-w-0 items-center gap-4">
          <span className="w-28 shrink-0 text-sm text-zinc-300">{date}</span>
          <select
            value={byDate[date] ?? ""}
            onChange={(e) => handleChange(date, e.target.value)}
            disabled={!!saving}
            className="min-w-[10rem] shrink-0 rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-white disabled:opacity-50"
            aria-label={`Status for ${date}`}
          >
            <option value="" style={{ backgroundColor: "#fff", color: "#18181b" }}>—</option>
            {OPTIONS.map((o) => (
              <option key={o.value} value={o.value} style={{ backgroundColor: "#fff", color: "#18181b" }}>
                {o.label}
              </option>
            ))}
          </select>
          {saving === date && <span className="shrink-0 text-xs text-zinc-500">Saving…</span>}
        </div>
      ))}
    </div>
  );
