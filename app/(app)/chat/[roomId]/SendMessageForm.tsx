...
export function SendMessageForm({ roomId }: { roomId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    const result = await sendMessage(roomId, trimmed, attachmentUrl);
    setLoading(false);
    if (result.error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2 border-t pt-4" style={{ borderColor: "var(--card-border)" }}>
      <input
        ref={fileInputRef}
        type="file"
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-zinc-500">{body.length} / 4000</span>
        </div>
        <button
          type="submit"
