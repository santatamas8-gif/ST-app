...
describe("getKioskLockRequestDecision with active lock and authenticated user", () => {
  ...
  it("allows Kiosk APIs and health checks", () => {
    ...
    expect(decide({ pathname: "/api/kiosk-match/create", method: "POST" })).toEqual({
      type: "allow",
    });
    expect(decide({ pathname: "/api/kiosk-match/submit", method: "POST" })).toEqual({
      type: "allow",
    });
    expect(decide({ pathname: "/api/kiosk-match/delete", method: "POST" })).toEqual({
      type: "allow",
    });
    expect(decide({ pathname: "/api/kiosk-match/add-participants", method: "POST" })).toEqual({
      type: "allow",
    });
