  ...
  async function uploadAvatar(userId: string, file: File) {
    ...
    try {
      ...
      if (res.ok && data.avatar_url) {
        ...
        setAvatarError(data.error ?? "Photo upload failed. Use JPG or PNG (max 5 MB).");
      }
    } catch {
      setAvatarError("Photo upload failed. Check your connection and try again.");
    } finally {
      setUploadingAvatarId(null);
    }
  }

  function openAvatarFilePicker(userId: string) {
    uploadTargetPlayerIdRef.current = userId;
...
{notes ? (
        ...
        {playersWithStatus.length > 0 && (
          ...
          >
            ...
            {avatarError && (
              <p className="mb-2 text-sm text-red-400">{avatarError}</p>
            )}
            {isAdmin && (
              <input
                ref={fileInputRef}
            ...
            <div className="relative hidden md:block [overflow-anchor:none]">
              ...
              >
                ...
                const isUploadingAvatar = uploadingAvatarId === p.id;
                        ...
                        {isAdmin ? (
                          <>
                            <button
                              ...
                              disabled={deletingAvatarId === p.id || isUploadingAvatar}
                              className={`relative block h-10 w-10 overflow-hidden rounded-full bg-zinc-700 ring-2 ring-zinc-600 transition-shadow hover:ring-emerald-500/50 md:h-12 md:w-12 ${deletingAvatarId === p.id || isUploadingAvatar ? "opacity-70" : ""}`}
                              title="Photo options"
                            >
                              {avatarUrl ? (
                                <img
                            ...
                            {isAvatarMenuOpen && (
                              <div className="absolute left-0 top-full z-[110] mt-1 min-w-[160px] rounded-lg border border-zinc-700 bg-zinc-900 py-1 shadow-xl">
                                <button
                                  type="button"
                                  onClick={() => openAvatarFilePicker(p.id)}
