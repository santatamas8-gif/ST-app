...
- Training Actual drill (planner-day ISO date, not server today): `date < 2026-09-01` → exactly `"Full Training"`; `date >= 2026-09-01` → exactly `"Full Training"` or `"Individual"`. Duplicate rows of the same drill → `ambiguous`. One Full Training plus one Individual from the cutoff → `found` as the metric-wise sum of two distinct loads. Never DISTINCT-all-rows. Never precedence. Do **not** use `Training_Drill_Switch`, `Top Up`, `SessionType = "Individual"`, or case/alias variants.
