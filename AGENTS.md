...
5. Reuse existing Power BI connector and query modules (`lib/powerbi/`, `getTrainingActualGps`, `getMatchBestGps`, `getMatchCandidateDates`, `getMatchActualGpsBatch`). Do not rebuild without explicit need. Total Load Match Actual is a **parallel** query; do **not** weaken Match filters. Training Actual drill (planner-day ISO date): `date < 2026-09-01` → exact `"Full Training"` only; `date >= 2026-09-01` → exact `"Full Training"` or `"Individual"`; duplicate rows of the same drill for the same player/day → `ambiguous`; one Full Training plus one Individual for the same player/day → `found` as the metric-wise sum of the two distinct loads; never DISTINCT-all-rows, never precedence, never sum duplicate same-drill rows. Match drill behavior remains unchanged. Official Matches are Admin-configured in Create/Edit Week (`planner_week_official_matches`, 0–2 rows); do not auto-resolve from week dates. See master spec **§E** / **§U3**.
